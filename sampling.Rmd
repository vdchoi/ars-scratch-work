...
exp_sampling = function(n, x_list) {
  ...
  # To fix - Need to add lower and upper bounds
  ...
  find_sample = function(slope_j, z_j, sum_prob, prob_indexed, 
                         constant_j, unif_draws) {
    
    if (abs(slope_j) < 0.00000001) {
      return(exp(upper[i_vector])*prob_indexed*sum_prob*unif_draws + z_j)
      
    }
    else {
      return(log(exp(slope_j*z_j) + 
            sum_prob*slope_j*prob_indexed*exp(-constant_j)*unif_draws) / slope_j)
    }
  }
  
  
  # To fix - Sometimes is negative
  print(exp(slope_j*z_j) + sum_prob*slope_j*prob_indexed*exp(-constant_j)*unif_draws)
